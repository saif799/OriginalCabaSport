...
Uploads are transformed at write time ([ADR-0007](docs/adr/0007-uploaded-images-are-transformed-at-write-time.md)): the browser downscales to 2000px ([lib/images/downscale.ts](lib/images/downscale.ts)), `POST /api/r2/upload` runs sharp, and three webp **Renditions** (400/800/1600) are stored — the uploaded file is not kept. A rendition is only re-encoded for the bytes it saves (ADR-0007 §8): every width is encoded, and where the upload is the better body — its own bytes at a width that would not resize them, or a lossless encode of a small `png` — that is stored instead unless it costs more than the **Quality Budget** (`QUALITY_BUDGET_BYTES`, 50 KB) over the encode. So a hand-optimised webp is stored as the exact bytes uploaded, and a 271 KB one whose encode saves 78 KB is not. Separately, the browser skips its downscale for a file under `ALREADY_SMALL_BYTES` (300 KB — the batch line the backfill selects on) that needs no resize, so sharp gets the original rather than a webp of it. All of it is decided by pure predicates in [lib/images/source.ts](lib/images/source.ts). `POST /api/r2/presigned-url` is now only the fallback for a browser that cannot downscale, and what it stores is a legacy single object.
